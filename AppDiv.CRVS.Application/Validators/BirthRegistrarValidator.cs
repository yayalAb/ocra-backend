...
namespace AppDiv.CRVS.Application.Validators
{
    public class BirthRegistrarValidator : AbstractValidator<RegistrarForBirthRequest>
    {
        ...
        public BirthRegistrarValidator(IEventRepository repo)
        {
            ...
            RuleFor(p => p.RegistrarInfo.FirstName.or).NotEmpty().NotNull().Matches("^[a-zA-Z']+${1,50}")
                .WithMessage("First Name should only contain alphabets and apostrophes, and be between 1 and 50 characters long.");
            RuleFor(p => p.RegistrarInfo.FirstName.am).NotEmpty().NotNull();
            RuleFor(p => p.RegistrarInfo.MiddleName.or).NotEmpty().NotNull().Matches("^[a-zA-Z']+${1,50}")
                .WithMessage("Middle Name should only contain alphabets and apostrophes, and be between 1 and 50 characters long.");
            RuleFor(p => p.RegistrarInfo.MiddleName.am).NotEmpty().NotNull();
            RuleFor(p => p.RegistrarInfo.LastName.or).NotEmpty().NotNull().Matches("^[a-zA-Z']+${1,50}")
                .WithMessage("Last Name should only contain alphabets and apostrophes, and be between 1 and 50 characters long.");
