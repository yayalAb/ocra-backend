...
namespace AppDiv.CRVS.Application.Validators
{
    public class BirthRegistrarValidator : AbstractValidator<RegistrarForBirthRequest>
    {
        ...
        public BirthRegistrarValidator(IEventRepository repo)
        {
            ...
            RuleFor(p => p.RegistrarInfo.SexLookupId.ToString()).NotEmpty().NotNull().ForeignKeyWithLookup(_repo, "Registrar.SexLookupId")
            ...
            RuleFor(p => p.RegistrarInfo.ResidentAddressId.ToString()).NotEmpty().NotNull().ForeignKeyWithAddress(_repo, "Registrar.ResidentAddressId")
