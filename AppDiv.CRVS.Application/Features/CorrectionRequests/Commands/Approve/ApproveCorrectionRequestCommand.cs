        ...
        public async Task<BaseResponse> Handle(ApproveCorrectionRequestCommand request, CancellationToken cancellationToken)
        {
            ...
            return await executionStrategy.ExecuteAsync(async () =>
            {
                ...
                using (var transaction = _CorrectionRequestRepostory.Database.BeginTransaction())
                {
                    ...
                    {
                        var response = await _WorkflowService.ApproveService(request.Id, "change", request.IsApprove, request.Comment, request.RejectionReasons, request.ReasonLookupId, false, cancellationToken);
                        if (response.Item1)
                        {
