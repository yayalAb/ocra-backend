...
namespace AppDiv.CRVS.Application.Features.DeathEvents.Command.Update
{
    public class UpdateDeathEventCommandValidator : AbstractValidator<UpdateDeathEventCommand>
    {
        ...
        {
            _repo = repo;
            RuleFor(p => CustomMapper.Mapper.Map<AddDeathEventRequest>(p)).SetValidator(new DeathEventValidator((_repo.Lookup, _repo.Person)));
            ...
            if (request.DeathNotification != null)
            {
                RuleFor(p => p.DeathNotification).SetValidator(new DeathNotificationValidator(_repo.Lookup));
            }
            RuleFor(p => p.Event.EventOwener.Id.ToString()).NotGuidEmpty().ForeignKeyWithPerson(_repo.Person, "EventOwener.Id");
            RuleFor(p => p.Event.EventOwener).SetValidator(new DeadValidator((_repo.Lookup, _repo.Address)));
            if (request.Event.EventRegistrar != null)
            {
                RuleFor(p => p.Event.EventRegistrar.RegistrarInfo.Id.ToString()).ForeignKeyWithPerson(_repo.Person, "EventRegistrar.RegistrarInfo.Id");
                RuleFor(p => p.Event.EventRegistrar).SetValidator(new DeathRegistrarValidator((_repo.Lookup, _repo.Address)));
            }
