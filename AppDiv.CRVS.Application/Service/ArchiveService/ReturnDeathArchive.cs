    ...
    public class ReturnDeathArchive : IReturnDeathArchive
    {
        ...
        public DeathArchiveDTO GetDeathArchive(Event death, string? BirthCertNo)
        {
            ...
            var deathInfo = new DeathArchiveDTO()
            {
             ...
             // deathInfo.Deceased.Age=death.DeathEventNavigation.Age.ToString()+" "+death.DeathEventNavigation.AgeUnit;
            if(death.EventDate!=null&&death.EventOwener.BirthDate!=null){
                ...
                if( death.EventDate.Year>BirthDate.Year){
                    if(death.EventDate.AddYears(-BirthDate.Year).Year==1&&death.EventDate.AddMonths(-BirthDate.Month).Month<0){
                     deathInfo.Deceased.Age=death.EventDate.AddMonths(-BirthDate.Month).Month.ToString();  
                    }else{
